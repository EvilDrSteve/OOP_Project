...
/**
 * @brief Counter where the Player drops off new Orders and picks up cooked food.
 *
 * Holds two Order* vectors (pendingOrders being cooked, readyOrders awaiting
 * pickup) and advances an Order's cook progress each update. Exposes its
 * occupied tiles so the Grid can treat them as solid for pathfinding.
 */
class Kitchen {
private:
