...
/**
 * @brief Central game world: owns the tile map, the player, all tables, the
 *        customer queue, the kitchen and the sink, and runs A*-style
 *        pathfinding between grid cells.
 *
 * Also handles mouse input (selecting tables, dragging customers from the
 * queue onto a table), routes the player to an interaction target and calls
 * the matching Player::interactWith* method when it arrives. Tracks per-round
 * stats (money, tips, tables served/lost).
 *
 * Note: Grid is deliberately the highest-traffic class in the project and
 * touches nearly every other module. A future refactor could split pathfinding,
 * input handling and bookkeeping into separate systems - see ARCHITECTURE.md.
 */
class Grid {
private:
