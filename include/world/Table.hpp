...
/**
 * @brief A restaurant table (2- or 4-seater) that a Customer can be dragged onto.
 *
 * Tracks its TableState (EMPTY / OCCUPIED / DIRTY), the seated Customer pointer,
 * and exposes getInteractionPoint() so the Grid knows where the Player should
 * stand to take an order or clear dishes.
 */
class Table {
private:
    bool occupied;
