...
/**
 * @brief Visible sprite for a single person in a customer group.
 *
 * A thin Character subclass that owns its own texture and positions itself in
 * pixel space given a grid size. Multiple CustomerCharacters are grouped under
 * one Customer (one per seat).
 */
...
/**
 * @brief A group of diners that moves through the dining state machine.
 *
 * Lifecycle (see CustomerState.hpp for the full enum):
 *   IN_QUEUE -> BEING_SEATED -> SEATED -> READY_TO_ORDER -> ORDER_TAKEN
 *   -> FOOD_DELIVERED -> READY_TO_PAY -> PAID -> LEFT
 *
 * The class owns the individual CustomerCharacter sprites, a patience timer
 * (customers get angry if ignored too long), indicators above their heads,
 * and the bill/tip amounts. The Player drives state transitions via
 * takeOrder() / receiveFood() / payBill() / clearDishes().
 */
class Customer {
private:
    int groupSize;
